...
class NeuralNet:
    """Wraps QuoridorNet with device handling, (un)batched inference and a
    single training step.  Returns/consumes numpy arrays so the rest of the
    code never touches torch directly."""

    def __init__(self, config, device=None):
        self.config = config
        self.device = device or resolve_device(config.device)
        self.net = QuoridorNet(config.board_size, config.channels,
                               config.res_blocks).to(self.device)
